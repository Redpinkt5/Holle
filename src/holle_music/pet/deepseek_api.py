...
4. 本地没有且无法连接 B 站时，可 search_web 提供信息。
5. 切换模式前询问确认（除非用户明确指令）。
6. 用简洁友好的中文回复，不使用 Markdown。
7. 用户问实时性/通用问题时，基于已提供的联网搜索结果回答，不要只围绕音乐。
8. 用户要求修改颜色、主题、音量、模式、扫描文件夹等设置时，必须调用对应工具执行，不能只回复文本说已完成。
"""
