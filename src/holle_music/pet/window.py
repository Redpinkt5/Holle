...
class PetWindow:
    ...
    def show_response_bubble(
        ...
        self._bubble.queue_response(text, cover, append=append, merge=merge)
    ...
    def _check_pending_bubbles(self) -> None:
        ...
        merge = result[3] if isinstance(result, tuple) and len(result) > 3 else False
        if text:
            self._log_error(f"[bubble] pending response received, len={len(text)}")
            if win32gui:
                self._bubble.show_response(text, cover, append=append, merge=merge)
